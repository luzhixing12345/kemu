...
static void virtio_bln_do_io(struct kvm *kvm, void *param)
{
	...
	if (vq == &g_bdev.vqs[VIRTIO_BLN_STATS]) {
		virtio_bln_do_stat_request(kvm, &g_bdev, vq);
		g_bdev.vdev.ops->signal_vq(kvm, &g_bdev.vdev, VIRTIO_BLN_STATS);
		return;
	}

	while (virt_queue__available(vq)) {
		virtio_bln_do_io_request(kvm, &g_bdev, vq);
		g_bdev.vdev.ops->signal_vq(kvm, &g_bdev.vdev, vq - g_bdev.vqs);
	}
}

static int virtio_bln__collect_stats(struct kvm *kvm)
{
	struct virt_queue *vq = &g_bdev.vqs[VIRTIO_BLN_STATS];
	u64 tmp;
	...
	virt_queue__set_used_elem(vq, g_bdev.cur_stat_head,
				  sizeof(struct virtio_balloon_stat));
	g_bdev.vdev.ops->signal_vq(kvm, &g_bdev.vdev, VIRTIO_BLN_STATS);

	if (read(g_bdev.stat_waitfd, &tmp, sizeof(tmp)) <= 0)
		return -EFAULT;
...
static void virtio_bln__print_stats(struct kvm *kvm, int fd, u32 type, u32 len, u8 *msg)
{
	...
	r = write(fd, g_bdev.stats, sizeof(g_bdev.stats));
	if (r < 0)
...
static void handle_mem(struct kvm *kvm, int fd, u32 type, u32 len, u8 *msg)
{
	...
	mem = *(int *)msg;
	num_pages = le32_to_cpu(g_bdev.config.num_pages);
	...
	g_bdev.config.num_pages = cpu_to_le32(num_pages);

	/* Notify that the configuration space has changed */
	g_bdev.vdev.ops->signal_config(kvm, &g_bdev.vdev);
}
...
int virtio_bln__init(struct kvm *kvm)
{
	...
	r = virtio_init(kvm, &g_bdev, &g_bdev.vdev, &bln_dev_virtio_ops,
