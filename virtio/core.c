...
static inline bool virt_desc__test_flag(struct virt_queue *vq, struct vring_desc *desc, u16 flag) {
 ...
 * end.
 */
static unsigned next_desc(struct virt_queue *vq, struct vring_desc *desc, unsigned int i, unsigned int max) {
    unsigned int next;
...
u16 virt_queue__get_head_iov(struct virt_queue *vq, struct iovec iov[], u16 *out, u16 *in, u16 head, struct kvm *kvm) {
    ...
    *out = *in = 0;
    ...
    do {
        ...
        iov[*out + *in].iov_base = guest_flat_to_host(kvm, virtio_guest_to_host_u64(vq->endian, desc[idx].addr));
...
bool virtio_queue__should_signal(struct virt_queue *vq) {
    ...
    if (!vq->use_event_idx) {
         ...
         */
        return !(virtio_guest_to_host_u16(vq->endian, vq->vring.avail->flags) & VRING_AVAIL_F_NO_INTERRUPT);
    }
...
void virtio_notify_status(struct kvm *kvm, struct virtio_device *vdev, void *dev, u8 status) {
    ...
    if ((status & VIRTIO_CONFIG_S_DRIVER_OK) && !(vdev->status & VIRTIO__STATUS_START)) {
...
int virtio_init(struct kvm *kvm, void *dev, struct virtio_device *vdev, struct virtio_ops *ops, enum virtio_trans trans,
