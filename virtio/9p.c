...
static void virtio_p9_error_reply(struct p9_dev *p9dev, struct p9_pdu *pdu, int err, u32 *outlen) {
    u16 tag;
...
static void virtio_p9_version(struct p9_dev *p9dev, struct p9_pdu *pdu, u32 *outlen) {
    u32 msize;
    char *version;
...
static void virtio_p9_clunk(struct p9_dev *p9dev, struct p9_pdu *pdu, u32 *outlen) {
    u32 fid;
...
static int get_full_path_helper(char *full_path, size_t size, const char *dirname, const char *name) {
    int ret;
...
static int get_full_path(char *full_path, size_t size, struct p9_fid *fid, const char *name) {
...
static void virtio_p9_open(struct p9_dev *p9dev, struct p9_pdu *pdu, u32 *outlen) {
    u32 fid, flags;
    struct stat st;
    struct p9_qid qid;
...
static void virtio_p9_create(struct p9_dev *p9dev, struct p9_pdu *pdu, u32 *outlen) {
    int fd, ret;
    char *name;
    size_t size;
    struct stat st;
...
static void virtio_p9_mkdir(struct p9_dev *p9dev, struct p9_pdu *pdu, u32 *outlen) {
    int ret;
    char *name;
    struct stat st;
    struct p9_qid qid;
...
static void virtio_p9_walk(struct p9_dev *p9dev, struct p9_pdu *pdu, u32 *outlen) {
    u8 i;
    u16 nwqid;
    u16 nwname;
    struct p9_qid wqid;
...
static void virtio_p9_attach(struct p9_dev *p9dev, struct p9_pdu *pdu, u32 *outlen) {
    char *uname;
    char *aname;
    struct stat st;
    struct p9_qid qid;
...
static void virtio_p9_fill_stat(struct p9_dev *p9dev, struct stat *st, struct p9_stat_dotl *statl) {
...
static void virtio_p9_read(struct p9_dev *p9dev, struct p9_pdu *pdu, u32 *outlen) {
    u64 offset;
    u32 fid_val;
    u16 iov_cnt;
    void *iov_base;
    ...
    pdu->in_iov_cnt = virtio_p9_update_iov_cnt(pdu->in_iov, count, pdu->in_iov_cnt);
...
static void virtio_p9_readdir(struct p9_dev *p9dev, struct p9_pdu *pdu, u32 *outlen) {
    u32 fid_val;
    u32 count, rcount;
    struct stat st;
    struct p9_fid *fid;
    ...
    while (dent) {
        ...
        virtio_p9_pdu_writef(pdu, "Qqbs", &qid, dent->d_off, dent->d_type, dent->d_name);
...
static void virtio_p9_getattr(struct p9_dev *p9dev, struct p9_pdu *pdu, u32 *outlen) {
    u32 fid_val;
    struct stat st;
    u64 request_mask;
    struct p9_fid *fid;
...
static void virtio_p9_setattr(struct p9_dev *p9dev, struct p9_pdu *pdu, u32 *outlen) {
    int ret = 0;
    u32 fid_val;
    struct p9_fid *fid;
    ...
    if ((p9attr.valid & (ATTR_UID | ATTR_GID)) ||
        ((p9attr.valid & ATTR_CTIME) && !((p9attr.valid & ATTR_MASK) & ~ATTR_CTIME))) {
...
static void virtio_p9_write(struct p9_dev *p9dev, struct p9_pdu *pdu, u32 *outlen) {
    u64 offset;
    u32 fid_val;
    u32 count;
    ssize_t res;
    ...
    pdu->out_iov_cnt = virtio_p9_update_iov_cnt(pdu->out_iov, count, pdu->out_iov_cnt);
...
static void virtio_p9_remove(struct p9_dev *p9dev, struct p9_pdu *pdu, u32 *outlen) {
    int ret;
    u32 fid_val;
    struct p9_fid *fid;
...
static void virtio_p9_rename(struct p9_dev *p9dev, struct p9_pdu *pdu, u32 *outlen) {
    int ret;
...
static void virtio_p9_readlink(struct p9_dev *p9dev, struct p9_pdu *pdu, u32 *outlen) {
    int ret;
    u32 fid_val;
    struct p9_fid *fid;
...
static void virtio_p9_statfs(struct p9_dev *p9dev, struct p9_pdu *pdu, u32 *outlen) {
    int ret;
    u64 fsid;
    u32 fid_val;
    struct p9_fid *fid;
    ...
    fsid = (unsigned int)stat_buf.f_fsid.__val[0] | (unsigned long long)stat_buf.f_fsid.__val[1] << 32;
...
static void virtio_p9_mknod(struct p9_dev *p9dev, struct p9_pdu *pdu, u32 *outlen) {
    int ret;
    char *name;
    struct stat st;
    struct p9_fid *dfid;
    ...
    virtio_p9_pdu_readf(pdu, "dsdddd", &fid_val, &name, &mode, &major, &minor, &gid);
...
static void virtio_p9_fsync(struct p9_dev *p9dev, struct p9_pdu *pdu, u32 *outlen) {
    int ret, fd;
    struct p9_fid *fid;
...
static void virtio_p9_symlink(struct p9_dev *p9dev, struct p9_pdu *pdu, u32 *outlen) {
    int ret;
    struct stat st;
    u32 fid_val, gid;
    struct p9_qid qid;
...
static void virtio_p9_link(struct p9_dev *p9dev, struct p9_pdu *pdu, u32 *outlen) {
    int ret;
    char *name;
...
static void virtio_p9_lock(struct p9_dev *p9dev, struct p9_pdu *pdu, u32 *outlen) {
    u8 ret;
    u32 fid_val;
...
static void virtio_p9_getlock(struct p9_dev *p9dev, struct p9_pdu *pdu, u32 *outlen) {
    u32 fid_val;
    struct p9_getlock glock;
    virtio_p9_pdu_readf(
        pdu, "dbqqds", &fid_val, &glock.type, &glock.start, &glock.length, &glock.proc_id, &glock.client_id);

    /* Just return success */
    glock.type = F_UNLCK;
    virtio_p9_pdu_writef(pdu, "bqqds", glock.type, glock.start, glock.length, glock.proc_id, glock.client_id);
    *outlen = pdu->write_offset;
    virtio_p9_set_reply_header(pdu, *outlen);
...
static void virtio_p9_renameat(struct p9_dev *p9dev, struct p9_pdu *pdu, u32 *outlen) {
    int ret;
    ...
    virtio_p9_pdu_readf(pdu, "dsds", &old_dfid_val, &old_name, &new_dfid_val, &new_name);
...
static void virtio_p9_unlinkat(struct p9_dev *p9dev, struct p9_pdu *pdu, u32 *outlen) {
    int ret;
    char *name;
    u32 fid_val, flags;
    struct p9_fid *fid;
...
static void virtio_p9_flush(struct p9_dev *p9dev, struct p9_pdu *pdu, u32 *outlen) {
    u16 tag, oldtag;
...
static void virtio_p9_eopnotsupp(struct p9_dev *p9dev, struct p9_pdu *pdu, u32 *outlen) {
...
static struct p9_pdu *virtio_p9_pdu_init(struct kvm *kvm, struct virt_queue *vq) {
    ...
    pdu->write_offset = VIRTIO_9P_HDR_LEN;
    pdu->queue_head =
        virt_queue__get_inout_iov(kvm, vq, pdu->in_iov, pdu->out_iov, &pdu->in_iov_cnt, &pdu->out_iov_cnt);
    return pdu;
}
...
static bool virtio_p9_do_io_request(struct kvm *kvm, struct p9_dev_job *job) {
    ...
    if ((cmd >= ARRAY_SIZE(virtio_9p_dotl_handler)) || !virtio_9p_dotl_handler[cmd])
...
static void notify_status(struct kvm *kvm, void *dev, u32 status) {
    ...
    if (status & VIRTIO__STATUS_CONFIG)
        p9dev->config->tag_len = virtio_host_to_guest_u16(p9dev->vdev.endian, p9dev->tag_len);

    if (!(status & VIRTIO__STATUS_STOP))
        return;

    rbtree_postorder_for_each_entry_safe(pfid, next, &p9dev->fids, node) close_fid(p9dev, pfid->fid);
}
...
int virtio_9p_img_name_parser(const struct option *opt, const char *arg, int unset) {
    ...
    if (stat(arg, &st) == 0 && S_ISDIR(st.st_mode)) {
        ...
        if (realpath(arg, tmp) == 0 || virtio_9p__register(kvm, tmp, "/dev/root") < 0)
            die("Unable to initialize virtio 9p");
    ...
    if (stat(path, &st) == 0 && S_ISDIR(st.st_mode)) {
        ...
        if (realpath(path, tmp) == 0 || virtio_9p__register(kvm, tmp, "/dev/root") < 0)
            die("Unable to initialize virtio 9p");
