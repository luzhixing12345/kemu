    ...
    while (1) {
        ...
        while (virt_queue__available(vq)) {
            ...
            while (copied < len) {
                ...
                if (copied == len)
                    break;
                while (!virt_queue__available(vq)) sleep(0);
                head = virt_queue__get_iov(vq, iov, &out, &in, kvm);
            }
...
static virtio_net_ctrl_ack virtio_net_handle_mq(struct kvm *kvm, struct net_dev *ndev,
...
static int virtio_net_request_tap(struct net_dev *ndev, struct ifreq *ifr, const char *tapname) {
    int ret;
...
static u64 get_host_features(struct kvm *kvm, void *dev) {
    ...
    features = 1UL << VIRTIO_NET_F_MAC | 1UL << VIRTIO_NET_F_CSUM | 1UL << VIRTIO_NET_F_HOST_TSO4 |
               1UL << VIRTIO_NET_F_HOST_TSO6 | 1UL << VIRTIO_NET_F_GUEST_TSO4 | 1UL << VIRTIO_NET_F_GUEST_TSO6 |
               1UL << VIRTIO_RING_F_EVENT_IDX | 1UL << VIRTIO_RING_F_INDIRECT_DESC | 1UL << VIRTIO_NET_F_CTRL_VQ |
               1UL << VIRTIO_NET_F_MRG_RXBUF | 1UL << (ndev->queue_pairs > 1 ? VIRTIO_NET_F_MQ : 0) |
...
static void virtio_net_update_endian(struct net_dev *ndev) {
    ...
    conf->status = virtio_host_to_guest_u16(ndev->vdev.endian, VIRTIO_NET_S_LINK_UP);
    conf->max_virtqueue_pairs = virtio_host_to_guest_u16(ndev->vdev.endian, ndev->queue_pairs);
...
static inline void str_to_mac(const char *str, char *mac) {
    sscanf(str, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", mac, mac + 1, mac + 2, mac + 3, mac + 4, mac + 5);
}
static int set_net_param(struct kvm *kvm, struct virtio_net_params *p, const char *param, const char *val) {
    ...
    *ops = net_dev_virtio_ops;
    ...
    if (params->trans) {
        if (strcmp(params->trans, "mmio") == 0)
            ...
            pr_warning(
                "virtio-net: Unknown transport method : %s, "
                "falling back to %s.",
                params->trans,
