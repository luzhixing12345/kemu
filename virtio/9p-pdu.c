...
static void virtio_p9_pdu_write(struct p9_pdu *pdu, const void *data, size_t size) {
    size_t len;
    int i, copied = 0;
    ...
    for (ptr = fmt; *ptr; ptr++) {
        switch (*ptr) {
            ...
            case 'Q': {
                ...
                retval = virtio_p9_pdu_readf(pdu, "bdq", &qid->type, &qid->version, &qid->path);
            } break;
            case 'S': {
                ...
                retval = virtio_p9_pdu_readf(pdu,
                                             "wwdQdddqssss",
                                             &stbuf->size,
                                             &stbuf->type,
                                             &stbuf->dev,
...
static int virtio_p9_pdu_encode(struct p9_pdu *pdu, const char *fmt, va_list ap) {
    ...
    for (ptr = fmt; *ptr; ptr++) {
        switch (*ptr) {
            ...
            case 'Q': {
                struct p9_qid *qid = va_arg(ap, struct p9_qid *);
                retval = virtio_p9_pdu_writef(pdu, "bdq", qid->type, qid->version, qid->path);
            } break;
            case 'S': {
                struct p9_wstat *stbuf = va_arg(ap, struct p9_wstat *);
