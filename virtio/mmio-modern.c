...
#define vmmio_selected_vq(vmmio) vdev->ops->get_vq((vmmio)->kvm, (vmmio)->dev, (vmmio)->hdr.queue_sel)

static void virtio_mmio_config_in(struct kvm_cpu *vcpu, u64 addr, u32 *data, u32 len, struct virtio_device *vdev) {
...
static void virtio_mmio_config_out(struct kvm_cpu *vcpu, u64 addr, u32 *data, u32 len, struct virtio_device *vdev) {
    struct virtio_mmio *vmmio = vdev->virtio;
...
void virtio_mmio_modern_callback(struct kvm_cpu *vcpu, u64 addr, u8 *data, u32 len, u8 is_write, void *ptr) {
    ...
    if (offset >= VIRTIO_MMIO_CONFIG) {
        ...
        virtio_access_config(vmmio->kvm, vdev, vmmio->dev, offset, data, len, is_write);
        return;
    }

    if (len != 4) {
        pr_debug("Invalid %s size %d at 0x%llx", is_write ? "write" : "read", len, addr);
        return;
    }
