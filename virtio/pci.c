...
int virtio_pci__init_ioeventfd(struct kvm *kvm, struct virtio_device *vdev, u32 vq) {
    ...
    if (vdev->ops->notify_vq_eventfd)
        vdev->ops->notify_vq_eventfd(kvm, vpci->dev, vq, vdev->legacy ? pio_fd : mmio_fd);
    return 0;
...
static void virtio_pci__msix_mmio_callback(struct kvm_cpu *vcpu, u64 addr, u8 *data, u32 len, u8 is_write, void *ptr) {
...
static int virtio_pci__bar_activate(struct kvm *kvm, struct pci_device_header *pci_hdr, int bar_num, void *data) {
...
static int virtio_pci__bar_deactivate(struct kvm *kvm, struct pci_device_header *pci_hdr, int bar_num, void *data) {
    u32 bar_addr;
    bool success;
    int r = -EINVAL;
...
int virtio_pci__init(struct kvm *kvm, void *dev, struct virtio_device *vdev, int device_id, int subsys_id, int class) {
    ...
    r = pci__register_bar_regions(kvm, &vpci->pci_hdr, virtio_pci__bar_activate, virtio_pci__bar_deactivate, vdev);
    if (r < 0)
        return r;
...
int virtio_pci__reset(struct kvm *kvm, struct virtio_device *vdev) {
    ...
    for (vq = 0; vq < vdev->ops->get_vq_count(kvm, vpci->dev); vq++) virtio_pci_exit_vq(kvm, vdev, vq);
