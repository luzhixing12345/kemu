...
#define vpci_selected_vq(vpci) vdev->ops->get_vq((vpci)->kvm, (vpci)->dev, (vpci)->queue_selector)

typedef bool (*access_handler_t)(struct virtio_device *, unsigned long, void *, int);

static bool virtio_pci__common_write(struct virtio_device *vdev, unsigned long offset, void *data, int size) {
    u64 features;
    u32 val, gsi, vec;
    ...
    switch (offset - VPCI_CFG_COMMON_START) {
        ...
        case VIRTIO_PCI_COMMON_Q_SIZE:
            vdev->ops->set_size_vq(vpci->kvm, vpci->dev, vpci->queue_selector, ioport__read16(data));
            break;
...
static bool virtio_pci__notify_write(struct virtio_device *vdev, unsigned long offset, void *data, int size) {
...
static bool virtio_pci__config_write(struct virtio_device *vdev, unsigned long offset, void *data, int size) {
    struct virtio_pci *vpci = vdev->virtio;

    return virtio_access_config(vpci->kvm, vdev, vpci->dev, offset - VPCI_CFG_DEV_START, data, size, true);
}

static bool virtio_pci__common_read(struct virtio_device *vdev, unsigned long offset, void *data, int size) {
    u32 val;
    struct virtio_pci *vpci = vdev->virtio;
...
static bool virtio_pci__isr_read(struct virtio_device *vdev, unsigned long offset, void *data, int size) {
    struct virtio_pci *vpci = vdev->virtio;
    ...
    kvm_irq_line(vpci->kvm, vpci->legacy_irq_line, VIRTIO_IRQ_LOW);
    vpci->isr = 0;

    return 0;
}

static bool virtio_pci__config_read(struct virtio_device *vdev, unsigned long offset, void *data, int size) {
    struct virtio_pci *vpci = vdev->virtio;

    return virtio_access_config(vpci->kvm, vdev, vpci->dev, offset - VPCI_CFG_DEV_START, data, size, false);
}

static bool virtio_pci_access(struct kvm_cpu *vcpu, struct virtio_device *vdev, unsigned long offset, void *data,
                              int size, bool write) {
...
void virtio_pci_modern__io_mmio_callback(struct kvm_cpu *vcpu, u64 addr, u8 *data, u32 len, u8 is_write, void *ptr) {
