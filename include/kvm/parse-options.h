...
#define OPT_UINTEGER(s, l, v, h)                                                                               \
    {                                                                                                          \
        .type = OPTION_UINTEGER, .short_name = (s), .long_name = (l), .value = check_vtype(v, unsigned int *), \
        .help = (h)                                                                                            \
    }
...
#define OPT_STRING(s, l, v, a, h)                                                                           \
    {                                                                                                       \
        .type = OPTION_STRING, .short_name = (s), .long_name = (l), .value = check_vtype(v, const char **), \
        .argh = (a), .help = (h)                                                                            \
    }
...
#define OPT_CALLBACK(s, l, v, a, h, f, p)                                                                     \
    {                                                                                                         \
        .type = OPTION_CALLBACK, .short_name = (s), .long_name = (l), .value = (v), .argh = (a), .help = (h), \
        .callback = (f), .ptr = (p),                                                                          \
    }
...
#define OPT_CALLBACK_NOOPT(s, l, v, a, h, f, p)                                                               \
    {                                                                                                         \
        .type = OPTION_CALLBACK, .short_name = (s), .long_name = (l), .value = (v), .argh = (a), .help = (h), \
        .callback = (f), .flags = PARSE_OPT_NOARG, .ptr = (p),                                                \
    }

#define OPT_CALLBACK_DEFAULT(s, l, v, a, h, f, d, p)                                                                   \
    {                                                                                                                  \
        .type = OPTION_CALLBACK, .short_name = (s), .long_name = (l), .value = (v), (a), .help = (h), .callback = (f), \
        .defval = (intptr_t)d, .flags = PARSE_OPT_LASTARG_DEFAULT, .ptr = (p)                                          \
    }

#define OPT_END() \
    { .type = OPTION_END }

#define OPT_ARCH(cmd, cfg) OPT_ARCH_##cmd(OPT_GROUP("Arch-specific options:"), &(cfg)->arch)
...
/* global functions */
void usage_with_options(const char *const *usagestr, const struct option *opts) NORETURN;
