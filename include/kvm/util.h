...
#define pr_debug(fmt, ...)                                                               \
    do {                                                                                 \
        if (loglevel >= LOGLEVEL_DEBUG)                                                  \
            __pr_debug("(%s) %s:%d: " fmt, __FILE__, __func__, __LINE__, ##__VA_ARGS__); \
    } while (0)
...
#define DIE_IF(cnd)                                                                      \
    do {                                                                                 \
        if (cnd)                                                                         \
            die(" at (" __FILE__ ":" __stringify(__LINE__) "): " __stringify(cnd) "\n"); \
    } while (0)
...
#define WARN_ON(condition)                                                                                        \
    ({                                                                                                            \
        ...
        if (__ret_warn_on)                                                                                        \
            pr_warning("(%s) %s:%d: failed condition: %s", __FILE__, __func__, __LINE__, __stringify(condition)); \
        __ret_warn_on;                                                                                            \
    })
