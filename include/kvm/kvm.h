...
int __must_check kvm__register_iotrap(struct kvm *kvm, u64 phys_addr, u64 len, mmio_handler_fn mmio_fn, void *ptr,
                                      unsigned int flags);

static inline int __must_check kvm__register_mmio(struct kvm *kvm, u64 phys_addr, u64 phys_addr_len, bool coalesce,
                                                  mmio_handler_fn mmio_fn, void *ptr) {
    return kvm__register_iotrap(
        kvm, phys_addr, phys_addr_len, mmio_fn, ptr, DEVICE_BUS_MMIO | (coalesce ? IOTRAP_COALESCE : 0));
}
static inline int __must_check kvm__register_pio(struct kvm *kvm, u16 port, u16 len, mmio_handler_fn mmio_fn,
                                                 void *ptr) {
