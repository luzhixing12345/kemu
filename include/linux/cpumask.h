...
static inline bool cpumask_and(cpumask_t *dstp, cpumask_t *src1p, cpumask_t *src2p) {
    return bitmap_and(cpumask_bits(dstp), cpumask_bits(src1p), cpumask_bits(src2p), NR_CPUS);
}
...
#define for_each_cpu(cpu, maskp) for ((cpu) = -1; (cpu) = cpumask_next((cpu), (maskp)), (cpu) < NR_CPUS;)

static inline int cpulist_parse(const char *buf, cpumask_t *dstp) {
    return bitmap_parselist(buf, cpumask_bits(dstp), NR_CPUS);
}

static inline bool cpumask_subset(const cpumask_t *src1p, const cpumask_t *src2p) {
