...
#define msi_update_state(state, val, bit) (state) = (val) ? (state) | bit : (state) & ~bit;
#define msi_set_enabled(state, val)       msi_update_state(state, val, VFIO_PCI_MSI_STATE_ENABLED)
#define msi_set_masked(state, val)        msi_update_state(state, val, VFIO_PCI_MSI_STATE_MASKED)
#define msi_set_empty(state, val)         msi_update_state(state, val, VFIO_PCI_MSI_STATE_EMPTY)
...
static int vfio_pci_enable_msis(struct kvm *kvm, struct vfio_device *vdev, bool msix) {
    size_t i;
    int ret = 0;
    int *eventfds;
     ...
     */
    if (!msi_is_enabled(msis->host_state) || (!msi_is_masked(msis->guest_state) && msi_is_empty(msis->host_state))) {
        bool empty = true;
...
static int vfio_pci_disable_msis(struct kvm *kvm, struct vfio_device *vdev, bool msix) {
    int ret;
    struct vfio_pci_device *pdev = &vdev->pci;
    struct vfio_pci_msi_common *msis = msix ? &pdev->msix : &pdev->msi;
...
static int vfio_pci_update_msi_entry(struct kvm *kvm, struct vfio_device *vdev, struct vfio_pci_msi_entry *entry) {
    int ret;
    ...
    if (entry->gsi < 0) {
        ret = irq__add_msix_route(kvm, &entry->config.msg, vdev->dev_hdr.dev_num << 3);
        if (ret < 0) {
...
static void vfio_pci_msix_pba_access(struct kvm_cpu *vcpu, u64 addr, u8 *data, u32 len, u8 is_write, void *ptr) {
...
static void vfio_pci_msix_table_access(struct kvm_cpu *vcpu, u64 addr, u8 *data, u32 len, u8 is_write, void *ptr) {
    ...
    msi_set_masked(entry->guest_state, entry->config.ctrl & PCI_MSIX_ENTRY_CTRL_MASKBIT);
...
static void vfio_pci_msix_cap_write(struct kvm *kvm, struct vfio_device *vdev, u16 off, void *data, int sz) {
    struct vfio_pci_device *pdev = &vdev->pci;
...
static int vfio_pci_msi_vector_write(struct kvm *kvm, struct vfio_device *vdev, u16 off, u8 *data, u32 sz) {
    size_t i;
    u32 mask = 0;
...
static void vfio_pci_msi_cap_write(struct kvm *kvm, struct vfio_device *vdev, u16 off, u8 *data, u32 sz) {
    u8 ctrl;
    struct msi_msg msg;
    size_t i, nr_vectors;
    struct vfio_pci_msi_entry *entry;
...
static int vfio_pci_bar_activate(struct kvm *kvm, struct pci_device_header *pci_hdr, int bar_num, void *data) {
...
static int vfio_pci_bar_deactivate(struct kvm *kvm, struct pci_device_header *pci_hdr, int bar_num, void *data) {
    struct vfio_device *vdev = data;
    struct vfio_pci_device *pdev = &vdev->pci;
    struct vfio_pci_msix_pba *pba = &pdev->msix_pba;
    struct vfio_pci_msix_table *table = &pdev->msix_table;
...
static void vfio_pci_cfg_read(struct kvm *kvm, struct pci_device_header *pci_hdr, u16 offset, void *data, int sz) {
    ...
    if (pread(vdev->fd, base, sz, info->offset + offset) != sz)
        vfio_dev_warn(vdev, "failed to read %d bytes from Configuration Space at 0x%x", sz, offset);
}

static void vfio_pci_cfg_write(struct kvm *kvm, struct pci_device_header *pci_hdr, u16 offset, void *data, int sz) {
    struct vfio_region_info *info;
    struct vfio_pci_device *pdev;
    struct vfio_device *vdev;
    u32 tmp;
    ...
    if (pwrite(vdev->fd, data, sz, info->offset + offset) != sz)
        vfio_dev_warn(vdev, "Failed to write %d bytes to Configuration Space at 0x%x", sz, offset);
...
static int vfio_pci_add_cap(struct vfio_device *vdev, u8 *virt_hdr, struct pci_cap_hdr *cap, off_t pos) {
    ...
    if (!hdr->capabilities) {
        ...
        while (last->next) last = PCI_CAP(virt_hdr, last->next);
...
static int vfio_pci_fixup_cfg_space(struct vfio_device *vdev) {
    ...
    for (i = VFIO_PCI_BAR0_REGION_INDEX; i <= VFIO_PCI_BAR5_REGION_INDEX; ++i) {
        ...
        if (region->is_ioport) {
            base = (region->port_base & PCI_BASE_ADDRESS_IO_MASK) | PCI_BASE_ADDRESS_SPACE_IO;
        } else {
            base = (region->guest_phys_addr & PCI_BASE_ADDRESS_MEM_MASK) | PCI_BASE_ADDRESS_SPACE_MEMORY;
        }
    ...
    if (msix) {
        ...
        pdev->msix_pba.fd_offset = vdev->regions[pba_index].info.offset + pba_bar_offset;
...
static int vfio_pci_get_region_info(struct vfio_device *vdev, u32 index, struct vfio_region_info *info) {
    int ret;
...
static int vfio_pci_create_msix_table(struct kvm *kvm, struct vfio_device *vdev) {
    ...
    for (i = 0; i < nr_entries; i++) entries[i].config.ctrl = PCI_MSIX_ENTRY_CTRL_MASKBIT;
...
static int vfio_pci_create_msi_cap(struct kvm *kvm, struct vfio_pci_device *pdev) {
    ...
    pdev->msi.entries = calloc(pdev->msi.nr_entries, sizeof(struct vfio_pci_msi_entry));
    if (!pdev->msi.entries)
        return -ENOMEM;
...
static int vfio_pci_configure_bar(struct kvm *kvm, struct vfio_device *vdev, size_t nr) {
    int ret;
    u32 bar;
    size_t map_size;
    struct vfio_pci_device *pdev = &vdev->pci;
...
static int vfio_pci_configure_dev_regions(struct kvm *kvm, struct vfio_device *vdev) {
    int ret;
    u32 bar;
    size_t i;
    bool is_64bit = false;
    ...
    for (i = VFIO_PCI_BAR0_REGION_INDEX; i <= VFIO_PCI_BAR5_REGION_INDEX; ++i) {
        ...
        bar = pdev->hdr.bar[i];
        is_64bit =
            (bar & PCI_BASE_ADDRESS_SPACE) == PCI_BASE_ADDRESS_SPACE_MEMORY && bar & PCI_BASE_ADDRESS_MEM_TYPE_64;
    }
    ...
    return pci__register_bar_regions(kvm, &pdev->hdr, vfio_pci_bar_activate, vfio_pci_bar_deactivate, vdev);
}
...
static int vfio_pci_reserve_irq_fds(size_t num) {
    ...
    if (setrlimit(RLIMIT_NOFILE, &new_limit)) {
        ...
        pr_warning("not enough FDs for full MSI-X support (estimated need: %zu)", (size_t)(needed - fd_limit.rlim_cur));
    }

    return 0;
}

static int vfio_pci_init_msis(struct kvm *kvm, struct vfio_device *vdev, struct vfio_pci_msi_common *msis) {
    int ret;
    size_t i;
    int *eventfds;
    size_t irq_set_size;
...
void vfio_pci_teardown_device(struct kvm *kvm, struct vfio_device *vdev) {
    ...
    for (i = 0; i < vdev->info.num_regions; i++) vfio_unmap_region(kvm, &vdev->regions[i]);
