...
static int vfio_device_pci_parser(const struct option *opt, char *arg, struct vfio_device_params *dev) {
...
static bool vfio_ioport_in(struct vfio_region *region, u32 offset, void *data, int len) {
...
static bool vfio_ioport_out(struct vfio_region *region, u32 offset, void *data, int len) {
    struct vfio_device *vdev = region->vdev;
    ssize_t nr;
    u32 val;
    ...
    if (nr != len)
        vfio_dev_err(vdev, "could not write %d bytes to I/O port 0x%x", len, offset + region->port_base);

    return nr == len;
}

static void vfio_ioport_mmio(struct kvm_cpu *vcpu, u64 addr, u8 *data, u32 len, u8 is_write, void *ptr) {
...
static void vfio_mmio_access(struct kvm_cpu *vcpu, u64 addr, u8 *data, u32 len, u8 is_write, void *ptr) {
    u64 val;
    ssize_t nr;
    struct vfio_region *region = ptr;
    struct vfio_device *vdev = region->vdev;
...
err_report:
    vfio_dev_err(vdev, "could not %s %u bytes at 0x%x (0x%llx)", is_write ? "write" : "read", len, offset, addr);
}

static int vfio_setup_trap_region(struct kvm *kvm, struct vfio_device *vdev, struct vfio_region *region) {
    if (region->is_ioport) {
        int port;
...
int vfio_map_region(struct kvm *kvm, struct vfio_device *vdev, struct vfio_region *region) {
    ...
    base = mmap(NULL, region->info.size, prot, MAP_SHARED, vdev->fd, region->info.offset);
    if (base == MAP_FAILED) {
...
static int vfio_configure_device(struct kvm *kvm, struct vfio_device *vdev) {
    ...
    if (vdev->info.flags & VFIO_DEVICE_FLAGS_RESET && ioctl(vdev->fd, VFIO_DEVICE_RESET) < 0)
    ...
    vfio_dev_info(vdev, "assigned to device number 0x%x in group %lu", vdev->dev_hdr.dev_num, group->id);
...
static int vfio_configure_reserved_regions(struct kvm *kvm, struct vfio_group *group) {
    FILE *file;
    int ret = 0;
    char type[9];
    ...
    snprintf(filename, PATH_MAX, IOMMU_GROUP_DIR "/%lu/reserved_regions", group->id);
...
static struct vfio_group *vfio_group_get_for_dev(struct kvm *kvm, struct vfio_device *vdev) {
    int dirfd;
    ssize_t ret;
    char *group_name;
...
static int vfio_device_init(struct kvm *kvm, struct vfio_device *vdev) {
    ...
    ret = snprintf(dev_path, PATH_MAX, "/sys/bus/%s/devices/%s", vdev->params->bus, vdev->params->name);
    if (ret < 0 || ret == PATH_MAX)
        return -EINVAL;
...
static int vfio__exit(struct kvm *kvm) {
    ...
    for (i = 0; i < kvm->cfg.num_vfio_devices; i++) vfio_device_exit(kvm, &vfio_devices[i]);
