...
struct kvm_cpu *kvm_cpu__arch_init(struct kvm *kvm, unsigned long cpu_id) {
    ...
    if (!kvm->cfg.arch.sbi_ext_disabled[KVM_RISCV_SBI_EXT_DBCN]) {
        id = 1;
        reg.id = RISCV_SBI_EXT_REG(KVM_REG_RISCV_SBI_SINGLE, KVM_RISCV_SBI_EXT_DBCN);
        reg.addr = (unsigned long)&id;
        if (ioctl(vcpu->vcpu_fd, KVM_SET_ONE_REG, &reg) < 0)
            ...
            switch (vcpu->kvm_run->riscv_sbi.function_id) {
                ...
                case SBI_EXT_DBCN_CONSOLE_READ:
                    ...
                    while (str_start <= str_end) {
                        if (vcpu->kvm_run->riscv_sbi.function_id == SBI_EXT_DBCN_CONSOLE_WRITE) {
