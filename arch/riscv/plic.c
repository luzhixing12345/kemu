...
static u32 __plic_context_best_pending_irq(struct plic_state *s, struct plic_context *c) {
    u8 best_irq_prio = 0;
    ...
    for (i = 0; i < s->num_irq_word; i++) {
        ...
        for (j = 0; j < 32; j++) {
            irq = i * 32 + j;
            if ((s->num_irq <= irq) || !(c->irq_pending[i] & (1 << j)) || (c->irq_claimed[i] & (1 << j)))
                continue;
...
/* Note: Must be called with c->irq_lock held */
static u32 __plic_context_irq_claim(struct plic_state *s, struct plic_context *c) {
...
static void plic__context_enable_read(struct plic_state *s, struct plic_context *c, u64 offset, void *data) {
...
static void plic__context_enable_write(struct plic_state *s, struct plic_context *c, u64 offset, void *data) {
    u8 irq_prio;
    u32 i, irq, irq_mask;
    u32 irq_word = offset >> 2;
...
static void plic__context_read(struct plic_state *s, struct plic_context *c, u64 offset, void *data) {
    mutex_lock(&c->irq_lock);
...
static void plic__context_write(struct plic_state *s, struct plic_context *c, u64 offset, void *data) {
...
static void plic__generate_fdt_node(void *fdt, struct kvm *kvm) {
    ...
    _FDT(fdt_property(fdt, "interrupts-extended", irq_cells, sizeof(u32) * plic.num_context * 2));
