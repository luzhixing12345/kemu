...
void kvm_init_ram(struct kvm *kvm) {
    ...
    if (kvm->arch.ram_alloc_start == MAP_FAILED)
        die("Failed to map %lld bytes for guest memory (%d)", kvm->arch.ram_alloc_size, errno);
...
bool kvm_arch_load_kernel_image(struct kvm *kvm, int fd_kernel, int fd_initrd, const char *kernel_cmdline) {
    ...
    pr_debug("Placing fdt at 0x%llx - 0x%llx", kvm->arch.dtb_guest_start, host_to_guest_flat(kvm, limit));
    limit = pos;
    ...
    if (fd_initrd != -1) {
        ...
        pr_debug("Loaded initrd to 0x%llx (%llu bytes)", kvm->arch.initrd_guest_start, kvm->arch.initrd_size);
    } else {
...
bool kvm_load_firmware(struct kvm *kvm, const char *firmware_filename) {
    ...
    pr_debug("Loaded firmware to 0x%llx (%zd bytes)", kvm->arch.kern_guest_start, fw_sz);
    ...
    kvm->arch.dtb_guest_start = ALIGN(host_to_guest_flat(kvm, host_pos), FDT_ALIGN);
    pr_debug("Placing fdt at 0x%llx - 0x%llx", kvm->arch.dtb_guest_start, kvm->arch.dtb_guest_start + FDT_MAX_SIZE);
