...
static void generate_irq_prop(void *fdt, u8 irq, enum irq_type irq_type) {
    u32 irq_prop[] = {cpu_to_fdt32(GIC_FDT_IRQ_TYPE_SPI), cpu_to_fdt32(irq - GIC_SPI_IRQ_BASE), cpu_to_fdt32(irq_type)};
...
static int setup_fdt(struct kvm *kvm) {
    ...
    void (*generate_mmio_fdt_nodes)(void *, struct device_header *, void (*)(void *, u8, enum irq_type));
    void (*generate_cpu_peripheral_fdt_nodes)(void *, struct kvm *) = kvm->cpus[0]->generate_fdt_nodes;
    ...
    if (kvm->arch.initrd_size != 0) {
        ...
        u64 ird_end_prop = cpu_to_fdt64(kvm->arch.initrd_guest_start + kvm->arch.initrd_size);
    ...
    while (dev_hdr) {
        ...
        if (generate_mmio_fdt_nodes) {
            ...
            pr_debug("Missing FDT node generator for MMIO device %d", dev_hdr->dev_num);
        }
