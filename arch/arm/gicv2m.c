...
#define GICV2M_MSI_TYPER_VAL(start, nr) (((start)&GICV2M_SPI_MASK) << 16 | ((nr)&GICV2M_SPI_MASK))
 ...
 */
static int gicv2m_update_routing(struct kvm *kvm, struct kvm_irq_routing_entry *entry) {
    int spi;
...
static void gicv2m_mmio_callback(struct kvm_cpu *vcpu, u64 addr, u8 *data, u32 len, u8 is_write, void *ptr) {
    if (is_write)
        return;
