...
static int gic__create_irqchip(struct kvm *kvm) {
    ...
    struct kvm_arm_device_addr gic_addr[] = {
        ...
        [1] = {
            .id = KVM_VGIC_V2_ADDR_TYPE_CPU | (KVM_ARM_DEVICE_VGIC_V2 << KVM_ARM_DEVICE_ID_SHIFT),
...
late_init(gic__init_gic)
    ...
    u64 msi_prop[2];
    u64 reg_prop[] = {
        cpu_to_fdt64(ARM_GIC_DIST_BASE), cpu_to_fdt64(ARM_GIC_DIST_SIZE), 0, 0, /* to be filled */
    };
...
u32 gic__get_fdt_irq_cpumask(struct kvm *kvm) {
    /* Only for GICv2 */
    if (kvm->cfg.arch.irqchip == IRQCHIP_GICV3 || kvm->cfg.arch.irqchip == IRQCHIP_GICV3_ITS)
        return 0;
