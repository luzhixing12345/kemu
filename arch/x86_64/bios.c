...
#define DEFINE_BIOS_IRQ_HANDLER(_irq, _handler)                                                 \
    {                                                                                           \
        .irq = _irq, .address = BIOS_IRQ_PA_ADDR(_handler), .handler = BIOS_IRQ_FUNC(_handler), \
...
void setup_bios(struct kvm *kvm) {
    ...
    for (i = 0; i < ARRAY_SIZE(bios_irq_handlers); i++) setup_irq_handler(kvm, &bios_irq_handlers[i]);
