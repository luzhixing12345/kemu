...
static unsigned int gen_cpu_flag(unsigned int cpu, unsigned int ncpu) {
    ...
    return ((cpu < ncpu) ? CPU_ENABLED : 0) | ((cpu == 0) ? CPU_BOOTPROCESSOR : 0x00);
}
...
static void mptable_add_irq_src(struct mpc_intsrc *mpc_intsrc, u16 srcbusid, u16 srcbusirq, u16 dstapic, u16 dstirq) {
