...
static void rtas_ibm_read_pci_config(struct kvm_cpu *vcpu, uint32_t token, uint32_t nargs, target_ulong args,
...
static void rtas_read_pci_config(struct kvm_cpu *vcpu, uint32_t token, uint32_t nargs, target_ulong args, uint32_t nret,
                                 target_ulong rets) {
    uint32_t val;
    union pci_config_address addr = {.w = rtas_ld(vcpu->kvm, args, 0)};
    struct pci_device_header *dev = pci__find_dev(addr.device_number);
...
static void rtas_ibm_write_pci_config(struct kvm_cpu *vcpu, uint32_t token, uint32_t nargs, target_ulong args,
                                      uint32_t nret, target_ulong rets) {
    uint64_t buid = ((uint64_t)rtas_ld(vcpu->kvm, args, 1) << 32) | rtas_ld(vcpu->kvm, args, 2);
    union pci_config_address addr = {.w = rtas_ld(vcpu->kvm, args, 0)};
    struct pci_device_header *dev = pci__find_dev(addr.device_number);
...
static void rtas_write_pci_config(struct kvm_cpu *vcpu, uint32_t token, uint32_t nargs, target_ulong args,
                                  uint32_t nret, target_ulong rets) {
    union pci_config_address addr = {.w = rtas_ld(vcpu->kvm, args, 0)};
    struct pci_device_header *dev = pci__find_dev(addr.device_number);
    uint32_t size = rtas_ld(vcpu->kvm, args, 1);
...
void spapr_create_phb(struct kvm *kvm, const char *busname, uint64_t buid, uint64_t mem_win_addr, uint64_t mem_win_size,
...
int spapr_populate_pci_devices(struct kvm *kvm, uint32_t xics_phandle, void *fdt) {
    ...
    uint32_t interrupt_map_mask[] = {cpu_to_be32(of_pci_b_ddddd(-1) | of_pci_b_fff(-1)), 0x0, 0x0, 0x0};
    ...
    _FDT(fdt_setprop(fdt, bus_off, "interrupt-map-mask", &interrupt_map_mask, sizeof(interrupt_map_mask)));
    ...
    while (dev_hdr) {
        ...
        _FDT(fdt_setprop_cell(fdt, node_off, "vendor-id", le16_to_cpu(hdr->vendor_id)));
        _FDT(fdt_setprop_cell(fdt, node_off, "device-id", le16_to_cpu(hdr->device_id)));
        _FDT(fdt_setprop_cell(fdt, node_off, "revision-id", hdr->revision_id));
        _FDT(fdt_setprop_cell(
            fdt, node_off, "class-code", hdr->class[0] | (hdr->class[1] << 8) | (hdr->class[2] << 16)));
        _FDT(fdt_setprop_cell(fdt, node_off, "subsystem-id", le16_to_cpu(hdr->subsys_id)));
        _FDT(fdt_setprop_cell(fdt, node_off, "subsystem-vendor-id", le16_to_cpu(hdr->subsys_vendor_id)));

        /* Config space region comes first */
        reg[0].phys_hi = cpu_to_be32(of_pci_b_n(0) | of_pci_b_p(0) | of_pci_b_t(0) | of_pci_b_ss(OF_PCI_SS_CONFIG) |
                                     of_pci_b_bbbbbbbb(0) | of_pci_b_ddddd(devid) | of_pci_b_fff(fn));
        reg[0].addr = 0;
        reg[0].size = 0;
        ...
        for (i = 0; i < 6; ++i) {
            ...
            reg[n + 1].phys_hi = cpu_to_be32(of_pci_b_n(0) | of_pci_b_p(0) | of_pci_b_t(0) |
                                             of_pci_b_ss(bar_to_ss(le32_to_cpu(hdr->bar[i]))) | of_pci_b_bbbbbbbb(0) |
                                             of_pci_b_ddddd(devid) | of_pci_b_fff(fn) | of_pci_b_rrrrrrrr(bars[i]));
            reg[n + 1].size = cpu_to_be64(pci__bar_size(hdr, i));
            reg[n + 1].addr = 0;

            assigned_addresses[n].phys_hi = cpu_to_be32(
                of_pci_b_n(1) | of_pci_b_p(0) | of_pci_b_t(0) | of_pci_b_ss(bar_to_ss(le32_to_cpu(hdr->bar[i]))) |
                of_pci_b_bbbbbbbb(0) | of_pci_b_ddddd(devid) | of_pci_b_fff(fn) | of_pci_b_rrrrrrrr(bars[i]));
        ...
        _FDT(fdt_setprop(fdt, node_off, "assigned-addresses", assigned_addresses, sizeof(assigned_addresses[0]) * (n)));
    ...
    /* Write interrupt map */
    _FDT(fdt_setprop(fdt, bus_off, "interrupt-map", &interrupt_map, devices * sizeof(interrupt_map[0])));
