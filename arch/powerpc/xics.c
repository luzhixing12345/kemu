...
static void icp_irq(struct icp_state *icp, int server, int nr, uint8_t priority) {
    ...
    if ((priority >= CPPR(ss)) || (XISR(ss) && (ss->pending_priority <= priority))) {
...
static void ics_write_xive_msi(struct ics_state *ics, int nr, int server, uint8_t priority) {
    struct ics_irq_state *irq = ics->irqs + nr - ics->offset;
...
static target_ulong h_cppr(struct kvm_cpu *vcpu, target_ulong opcode, target_ulong *args) {
...
static target_ulong h_ipi(struct kvm_cpu *vcpu, target_ulong opcode, target_ulong *args) {
...
static target_ulong h_xirr(struct kvm_cpu *vcpu, target_ulong opcode, target_ulong *args) {
...
static target_ulong h_eoi(struct kvm_cpu *vcpu, target_ulong opcode, target_ulong *args) {
...
static void rtas_set_xive(struct kvm_cpu *vcpu, uint32_t token, uint32_t nargs, target_ulong args, uint32_t nret,
                          target_ulong rets) {
    ...
    if (!ics_valid_irq(ics, nr) || (server >= ics->icp->nr_servers) || (priority > 0xff)) {
        rtas_st(vcpu->kvm, rets, 0, -3);
        return;
    }
...
static void rtas_get_xive(struct kvm_cpu *vcpu, uint32_t token, uint32_t nargs, target_ulong args, uint32_t nret,
                          target_ulong rets) {
    struct ics_state *ics = vcpu->kvm->arch.icp->ics;
    uint32_t nr;
...
static void rtas_int_off(struct kvm_cpu *vcpu, uint32_t token, uint32_t nargs, target_ulong args, uint32_t nret,
                         target_ulong rets) {
    struct ics_state *ics = vcpu->kvm->arch.icp->ics;
    uint32_t nr;
...
static void rtas_int_on(struct kvm_cpu *vcpu, uint32_t token, uint32_t nargs, target_ulong args, uint32_t nret,
                        target_ulong rets) {
    struct ics_state *ics = vcpu->kvm->arch.icp->ics;
    uint32_t nr;
