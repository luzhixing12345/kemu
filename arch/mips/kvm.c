...
void kvm__arch_init(struct kvm *kvm) {
    ...
    kvm->ram_start = mmap_anon_or_hugetlbfs(kvm, kvm->cfg.hugetlbfs_path, kvm->ram_size);
...
static void kvm__mips_install_cmdline(struct kvm *kvm) {
    ...
    if ((u64)kvm->ram_size <= KVM_MMIO_START)
        sprintf(p + cmdline_offset, "mem=0x%llx@0 ", (unsigned long long)kvm->ram_size);
    else
        sprintf(p + cmdline_offset,
                "mem=0x%llx@0 mem=0x%llx@0x%llx ",
    ...
    while (p[cmdline_offset]) {
        if (!isspace(p[cmdline_offset])) {
            ...
            argc++;
            while (p[cmdline_offset] && !isspace(p[cmdline_offset])) cmdline_offset++;
            continue;
        }
...
static bool load_elf_binary(struct kvm *kvm, int fd_kernel) {
    ...
    if (eh.ehdr.e_ident[EI_MAG0] != ELFMAG0 || eh.ehdr.e_ident[EI_MAG1] != ELFMAG1 ||
        eh.ehdr.e_ident[EI_MAG2] != ELFMAG2 || eh.ehdr.e_ident[EI_MAG3] != ELFMAG3 ||
    ...
    pr_info("ELF Loading 0x%lx bytes from 0x%llx to 0x%llx",
            (unsigned long)ei.len,
            (unsigned long long)ei.offset,
