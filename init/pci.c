...
static void pci_config_address_mmio(struct kvm_cpu *vcpu, u64 addr, u8 *data, u32 len, u8 is_write, void *ptr) {
...
static void pci_config_data_mmio(struct kvm_cpu *vcpu, u64 addr, u8 *data, u32 len, u8 is_write, void *kvm) {
    union pci_config_address pci_config_address;
...
static int pci_activate_bar(struct kvm *kvm, struct pci_device_header *pci_hdr, int bar_num) {
    int r = 0;
...
static int pci_deactivate_bar(struct kvm *kvm, struct pci_device_header *pci_hdr, int bar_num) {
    int r = 0;
...
static void pci_config_command_wr(struct kvm *kvm, struct pci_device_header *pci_hdr, u16 new_command) {
    int i;
...
static void pci_config_bar_wr(struct kvm *kvm, struct pci_device_header *pci_hdr, int bar_num, u32 value) {
    ...
    if (pci__bar_is_memory(pci_hdr, bar_num) && !pci__memory_space_enabled(pci_hdr)) {
        pci_hdr->bar[bar_num] = value;
        return;
    }
...
static const u8 pci_config_writable[PCI_STD_HEADER_SIZEOF] = {
    [PCI_COMMAND] = PCI_COMMAND_IO | PCI_COMMAND_MEMORY | PCI_COMMAND_MASTER | PCI_COMMAND_PARITY,
...
static void pci_config_mmio_access(struct kvm_cpu *vcpu, u64 addr, u8 *data, u32 len, u8 is_write, void *kvm) {
...
int pci__register_bar_regions(struct kvm *kvm, struct pci_device_header *pci_hdr, bar_activate_fn_t bar_activate_fn,
