...
static const char *const run_usage[] = {"lkvm run [<options>] [<kernel image>]", NULL};
...
#define BUILD_OPTIONS(name, cfg, kvm)                                                                                \
    struct option name[] = {                                                                                         \
        ...
        OPT_STRING('\0', "name", &(cfg)->guest_name, "guest name", "A name for the guest"),                          \
        OPT_INTEGER('c', "cpus", &(cfg)->nrcpus, "Number of CPUs"),                                                  \
        OPT_CALLBACK('m', "mem", NULL, MEM_OPT_HELP_SHORT, MEM_OPT_HELP_LONG, mem_parser, kvm),                      \
        OPT_CALLBACK('d',                                                                                            \
                     "disk",                                                                                         \
                     kvm,                                                                                            \
                     "image or rootfs_dir",                                                                          \
        ...
        OPT_CALLBACK('\0', "tty", NULL, "tty id", "Remap guest TTY into a pty on the host", tty_parser, NULL),       \
        OPT_STRING('\0',                                                                                             \
                   "sandbox",                                                                                        \
                   &(cfg)->sandbox,                                                                                  \
                   "script",                                                                                         \
        ...
        OPT_CALLBACK('\0',                                                                                           \
                     ...
                     NULL),                                                                                          \
        OPT_CALLBACK(                                                                                                \
            '\0', "loglevel", NULL, "[error|warning|info|debug]", "Set the verbosity level", loglevel_parser, NULL), \
                                                                                                                     \
        ...
        OPT_STRING('p', "params", &(cfg)->kernel_cmdline, "params", "Kernel command line arguments"),                \
        OPT_STRING(                                                                                                  \
            'f', "firmware", &(cfg)->firmware_filename, "firmware", "Firmware image to boot in virtual machine"),    \
        OPT_STRING('F', "flash", &(cfg)->flash_filename, "flash", "Flash image to present to virtual machine"),      \
                                                                                                                     \
        OPT_GROUP("Networking options:"),                                                                            \
        OPT_CALLBACK_DEFAULT('n',                                                                                    \
                             "network",                                                                              \
                             NULL,                                                                                   \
                             "network params",                                                                       \
        ...
        OPT_CALLBACK_NOOPT('\0',                                                                                     \
                           ...
                           loglevel_parser,                                                                          \
                           NULL),                                                                                    \
        OPT_BOOLEAN('\0', "debug-single-step", &(cfg)->single_step, "Enable single stepping"),                       \
        OPT_BOOLEAN('\0', "debug-ioport", &(cfg)->ioport_debug, "Enable ioport debugging"),                          \
        OPT_BOOLEAN('\0', "debug-mmio", &(cfg)->mmio_debug, "Enable MMIO debugging"),                                \
        OPT_INTEGER('\0', "debug-iodelay", &(cfg)->debug_iodelay, "Delay IO by millisecond"),                        \
                                                                                                                     \
...
static const char *default_kernels[] = {
    "./bzImage", "arch/" BUILD_ARCH "/boot/bzImage", "../../arch/" BUILD_ARCH "/boot/bzImage", NULL};
...
static void kvm_write_sandbox_cmd_exactly(int fd, const char *arg) {
    ...
    while (*arg) {
        ...
        if (arg != single_quote) {
            if (write(fd, "'", 1) <= 0 || write(fd, arg, single_quote - arg) <= 0 || write(fd, "'", 1) <= 0)
                die("Failed writing sandbox script");
        }
...
static struct kvm *kvm_cmd_run_init(int argc, const char **argv) {
    ...
    while (argc != 0) {
        BUILD_OPTIONS(options, &kvm->cfg, kvm);
        argc = parse_options(argc, argv, options, run_usage, PARSE_OPT_STOP_AT_NON_OPTION | PARSE_OPT_KEEP_DASHDASH);
        if (argc != 0) {
    ...
    if (kvm->cfg.kernel_filename) {
        pr_info("# %s run -k %s -m %Lu -c %d --name %s",
                KVM_BINARY_NAME,
        ...
        pr_info("# %s run --firmware %s -m %Lu -c %d --name %s",
                KVM_BINARY_NAME,
