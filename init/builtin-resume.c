...
static const char *const resume_usage[] = {"lkvm resume [--all] [-n name]", NULL};

static const struct option resume_options[] = {OPT_GROUP("General options:"),
...
static void parse_resume_options(int argc, const char **argv) {
    while (argc != 0) {
        argc = parse_options(argc, argv, resume_options, resume_usage, PARSE_OPT_STOP_AT_NON_OPTION);
        if (argc != 0)
            kvm_resume_help();
    }
}
...
int kvm_cmd_resume(int argc, const char **argv, const char *prefix) {
    ...
    if (all)
        return kvm_enumerate_instances(do_resume);
