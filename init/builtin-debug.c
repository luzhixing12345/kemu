...
static const char *const debug_usage[] = {"lkvm debug [--all] [-n name] [-d] [-m vcpu]", NULL};

static const struct option debug_options[] = {OPT_GROUP("General options:"),
...
static void parse_debug_options(int argc, const char **argv) {
    while (argc != 0) {
        argc = parse_options(argc, argv, debug_options, debug_usage, PARSE_OPT_STOP_AT_NON_OPTION);
        if (argc != 0)
            kvm_debug_help();
    }
}
