...
static const struct option pause_options[] = {OPT_GROUP("General options:"),
...
static void parse_pause_options(int argc, const char **argv) {
    while (argc != 0) {
        argc = parse_options(argc, argv, pause_options, pause_usage, PARSE_OPT_STOP_AT_NON_OPTION);
        if (argc != 0)
            kvm_pause_help();
    }
}
...
int kvm_cmd_pause(int argc, const char **argv, const char *prefix) {
    ...
    if (all)
        return kvm_enumerate_instances(do_pause);
