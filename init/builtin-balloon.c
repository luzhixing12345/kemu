...
static const char *const balloon_usage[] = {"lkvm balloon [-n name] [-p pid] [-i amount] [-d amount]", NULL};
...
static void parse_balloon_options(int argc, const char **argv) {
    while (argc != 0) {
        argc = parse_options(argc, argv, balloon_options, balloon_usage, PARSE_OPT_STOP_AT_NON_OPTION);
        if (argc != 0)
            kvm_balloon_help();
    }
}
...
int kvm_cmd_balloon(int argc, const char **argv, const char *prefix) {
    ...
    r = kvm_ipc__send_msg(instance, KVM_IPC_BALLOON, sizeof(amount), (u8 *)&amount);
