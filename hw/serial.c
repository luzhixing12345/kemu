...
#define SERIAL_REGS_SETTING                                                                                          \
    .iir = UART_IIR_NO_INT, .lsr = UART_LSR_TEMT | UART_LSR_THRE, .msr = UART_MSR_DCD | UART_MSR_DSR | UART_MSR_CTS, \
    .mcr = UART_MCR_OUT2,

#ifdef CONFIG_HAS_LIBFDT
static void serial8250_generate_fdt_node(void *fdt, struct device_header *dev_hdr, fdt_irq_fn irq_fn);
#else
...
static void serial8250__receive(struct kvm *kvm, struct serial8250_device *dev, bool handle_sysrq) {
    int c;
    ...
    if (kvm->cfg.device.active_console != CONSOLE_8250)
        return;
...
static bool serial8250_in(struct serial8250_device *dev, struct kvm_cpu *vcpu, u16 offset, void *data) {
    bool ret = true;
...
static void serial8250_generate_fdt_node(void *fdt, struct device_header *dev_hdr, fdt_irq_fn irq_fn) {
    ...
    struct serial8250_device *dev = container_of(dev_hdr, struct serial8250_device, dev_hdr);
