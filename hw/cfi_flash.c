...
#define PROGRAM_BUFF_SIZE_BITS_PER_CHIP (PROGRAM_BUFF_SIZE_BITS + 1 - CFI_NR_FLASH_CHIPS)
...
#define CFI_STATUS_LOCK_ERROR           CFI_STATUS_PROGRAM_LOCK_BIT | CFI_STATUS_PROTECT_BIT
#define CFI_STATUS_ERASE_ERROR          CFI_STATUS_ERASE_CLEAR_LOCK_BIT | CFI_STATUS_PROGRAM_LOCK_BIT
...
static void word_program(struct cfi_flash_device *sfdev, u64 faddr, void *data, int len) {
...
static bool buffer_write(struct cfi_flash_device *sfdev, u64 faddr, void *buffer, int len) {
...
static void buffer_confirm(struct cfi_flash_device *sfdev) {
    ...
    memcpy(sfdev->flash_memory + sfdev->block_address, sfdev->program_buffer, sfdev->buff_written);
}
...
static void cfi_flash_read(struct cfi_flash_device *sfdev, u64 faddr, u8 *data, u32 len) {
    u16 cfi_value = 0;
    ...
    switch (len) {
        ...
        case 2:
            ...
            break;
        default:
            pr_debug("CFI flash: illegal access length %d for read mode %d", len, sfdev->read_mode);
            break;
    }
}
...
static void cfi_flash_write(struct cfi_flash_device *sfdev, u16 command, u64 faddr, u8 *data, u32 len) {
    switch (sfdev->state) {
        case READY:
        ...
        case BUFFER_WRITE:
            ...
            if ((command & 0xff) == CFI_CMD_CONFIRM) {
                ...
                pr_debug("CFI flash: BUFFER_WRITE: expected CONFIRM(0xd0), got 0x%x @ 0x%llx", command, faddr);
...
static void cfi_flash_mmio(struct kvm_cpu *vcpu, u64 addr, u8 *data, u32 len, u8 is_write, void *context) {
...
static void generate_cfi_flash_fdt_node(void *fdt, struct device_header *dev_hdr,
...
static struct cfi_flash_device *create_flash_device_file(struct kvm *kvm, const char *filename) {
    struct cfi_flash_device *sfdev;
    struct stat statbuf;
    unsigned int value;
    int ret;
    ...
    if (sfdev->size < statbuf.st_size) {
        pr_info("flash file size (%llu bytes) is not a power of two", (unsigned long long)statbuf.st_size);
        pr_info("only using first %u bytes", sfdev->size);
    }
    sfdev->flash_memory = mmap(NULL, sfdev->size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
